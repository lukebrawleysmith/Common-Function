...
/******* common functions *******/
void SetEqual(int *length_x, double *x, double *copy_x) {
/********************************************************************
...
void FindIntegerMax(int *length_x, int *x, int *max_x) {
/********************************************************************
...
void FindInterval(int *length_x, double *x, double *tau, int *bin) {
/********************************************************************
...
void MatrixMultiply (int *t_a, int *t_b, int *nrow_a, int *ncol_a, int *nrow_b,
...
**********************************************************************/
  ...
  MatrixMultiply(&t_a, &t_b, nrow_e, nrow_e, nrow_e, nrow_d, e, x, ex);
  MatrixMultiply(&t_a, &t_b, nrow_e, nrow_d, nrow_d, nrow_d, ex, d, exd);
...
void ISpline(double *tau, int *spline_df, int *spline_knots_length,
  ...
  ******************************************************/
  ...
  int bin = 2;
  FindInterval(spline_knots_length, spline_knots, tau, &bin);
...
void LogDInverseWishart(int *p, double *df,
  ...
  **********************************************************************/
  ...
  log_det *= -1.0;
  MatrixMultiply(&int_zero, &int_zero, p, p, p, p, sai, x_inv, product_matrix);
  for(i = 0; i < *p; i++) {
...
void MakeBasis (int *basis_ind, int *n_bf, double *basis_knots, int *n_tau,
  ...
  *************************************************/
  ...
  if (*basis_ind == -1) {
    ...
    for (i = 0; i < *n_tau; i++) {
      FindInterval(&knots_length, basis_knots, &tau[i], &dummy_bin);
      for (j = 1; j < *n_bf; j++) {
        k = j - 1;
...
//  Rprintf("*rho rho_1, z, sd_rho, can_rho_prime, can_rho_1, can_rho, *max_rho = %f %f %f %f %f %f %f %f\n", *rho, rho_1, z, *sd_rho, can_rho_prime, can_rho_1, can_rho, *max_rho);
  ...
  if(e > (rho_post  - can_rho_post)) {
    *rho = can_rho;
    SetEqual(&length_prec_clusters, can_prec_clusters, prec_clusters);
    InvertSymmetricMatrix(&n_clusters, prec_clusters,
      cov_clusters, &dummy_log_det);
    SetEqual(&length_prec_thetastar, can_prec_thetastar, prec_thetastar);
...
void GenerateResponse(int *dim_thetastar, int *n_y, int *basis_ind,
  ...
  ************************************************/
  ...
  int max_n_y = 0;
  FindIntegerMax(&n_clusters, n_y, &max_n_y);
  ...
  for(j = 0; j < n_timepoints; j++) {
    for(g = 0; g < n_clusters; g++) {
      for(h = 0; h < n_responses; h++) {
        ...
        if(*tau_low > 0.0) {
          if(basis_ind[h] == -1) {
            MatrixMultiply(&int_one, &int_zero, &n_bf, &n_preds, &n_bf,
        ...
        MatrixMultiply(&int_zero, &int_one, &max_n_y, &n_preds, &n_bf,
...
void MiddleSplineDensity(int *n_bf, int *n_preds, double *y, double *u,
  ...
  ************************************************************/
  ...
  FindInterval(&length_breaks, w, y, bin);
  for(m = 0; m < *n_bf; m++) {
    w[m] = 0.0;
    for(p = 0; p < *n_preds; p++) {
  ...
  ******************************************************************/
  ...
  if(*n_bf == 2) {
    *bin = 0;
    ...
    FindInterval(n_bf, breaks, y, bin);
  }
  scale = 0.0;
  for(p = 0; p < *n_preds; p++) {
...
void Likelihood(int *dim_thetastar, int *n_y, int *basis_ind,
  ...
  *ll_sum = 0.0;
  for(j = 0; j < n_timepoints; j++) {
    ...
    MatrixMultiply(&int_zero, &int_zero, &nrow_this_basis, &n_bf, &n_bf,
      &n_preds, this_basis, this_theta, break_matrix);
    MatrixMultiply(&int_one, &int_zero, &n_bf, &n_preds, &n_bf, &int_one,
  ...
  **********************************************************/
  ...
  for(g = 0; g < n_clusters; g++) {
    for(i = 0; i < n_y[g]; i++) {
      ...
      MakeZ(dim_thetastar, n_preds_q, this_x, z);
      MatrixMultiply(&int_zero, &int_zero, &n_timepoints_responses,
  ...
  **********************************************************/
  ...
  for(g = 0; g < n_clusters; g++) {
    for(i = 0; i < n_y[g]; i++) {
      ...
      MakeZ(dim_thetastar, n_preds_q, this_x, z);
      MatrixMultiply(&int_one, &int_zero, &n_row_z, &n_col_z,
      ...
      MatrixMultiply(&int_one, &int_zero, &n_row_z, &n_col_z, &n_row_z,
        &int_one, z, omega, omega2);
      MatrixMultiply(&int_zero, &int_zero, &n_col_z, &n_col_z, &n_col_z,
...
//        }
      ...
      MatrixMultiply(&int_zero, &int_zero, &n_row_z, &n_col_z, &n_col_z,
...
//        }
      ...
      MatrixMultiply(&int_zero, &int_zero, &n_row_z, &n_row_z, &n_row_z,
...
void make_b (
  ...
  if (*shape < 0) {
    ...
    for (i = 0; i < kappa_length; i++) {
      FindInterval(&kappa_length, basis_knots, &basis_knots[i], &dummy_bin);
      for (j = 1; j < *n_bf; j++) {
        k = j - 1;
  ...
  for (i = 0; i < *n_gjh; i++) {
    ...
    if (status[this_i] == 0 || status[this_i] == 1) {
      ...
      if (dummy_low > y[this_i] || dummy_high < y[this_i]) {
        ...
        FindInterval(&kappa_length, breaks, &y[this_i], &bin[this_i]);
      }
      a = 0.0;
      b = 0.0;
      c = 0.0;
    ...
    if (status[this_i] == 2 || status[this_i] == 4) {
      ...
      if (dummy_low > y_high[this_i] || dummy_high < y_high[this_i]) {
        ...
        FindInterval(&kappa_length, breaks, &y_high[this_i], &bin_high[this_i]);
      }
      a = 0.0;
      b = 0.0;
      c = 0.0;
    ...
    if (status[this_i] == 3 || status[this_i] == 4) {
      ...
      if (dummy_low > y_low[this_i] || dummy_high < y_low[this_i]) {
        ...
        FindInterval(&kappa_length, breaks, &y_low[this_i], &bin_low[this_i]);
      }
      a = 0.0;
      b = 0.0;
      c = 0.0;
...
void clike (int *G, int *J, int *H, int *M, int *P,
  ...
  if (*tau_high < 1) {
    ...
    if (*M > 2) {
      ...
      int T_B = 0;
      int dummy_one = 1;
//      MatrixMultiply(&T_A, &T_B, P, &dummy_one, &M_1, dummy_mat, dummy_vec, mn_2);
      MatrixMultiply(&T_A, &T_B, &M_1, P, P, &dummy_one, dummy_mat, dummy_vec, mn_2);
      for (p = 0; p < *P; p++) {
  ...
  for (i = 0; i < *n_gjh; i++) {
    ...
    if (status[this_i] == 0 || status[this_i] == 1) { /*uncensored observations*/
      if (low_thresh > y[this_i]) {
        ...
        FindInterval(M, breaks, &y[this_i], &bin[this_i]);
        s = 0;
        if (bin[this_i] == 0) {
          mn = 0;
          for (p = 0; p < *P; p++) {
      ...
      if (status[this_i] == 2 || status[this_i] == 4) {
        ...
        FindInterval(M, breaks, &y_high[this_i], &bin_high[this_i]);
        s = 0;
      ...
      FindInterval(M, breaks, &y_low[this_i], &bin_low[this_i]);
      s = 0;
      if (bin_low[this_i] == 0) {
        mn = 0;
        for (p = 0; p < *P; p++) {
  ...
  for (i = 0; i < *n_gjh; i++) {
    ...
    if (status[this_i] == 0) { /*missing observations*/
      if (U[this_i] < *tau_low) {
        ...
        FindInterval(M, kappa, &U[this_i], &bin[this_i]);
        s = 0;
        if (bin[this_i] == 0) {
          mn = 0;
          for (p = 0; p < *P; p++) {
  ...
  for (i = 0; i < *n_gjh; i++) {
    ...
    if (status[this_i] == 0) {
      if (*tau_low > U[this_i]) {
        ...
        FindInterval(&kappa_length, kappa, &U[this_i], &bin[this_i]);
        a = 0.0;
        b = 0.0;
        c = 0.0;
        d = w[0];
...
void gibbs_mu(int *G, int *D, int *H, int *M, int *P, int *d, int *m, int *p,
  ...
  MatrixMultiply(&T_A, &T_B, H, &dim_ll, &dim_ll, &dummy_one, dummy_matrix, this_theta, dummy_mean_mu);
//  MatrixMultiply(&T_A, &T_B, H, &dummy_one, &dim_ll, dummy_matrix, this_theta, dummy_mean_mu);
  ...
  MatrixMultiply(&T_A, &T_B, H, H, H, &dummy_one, cov_mu, dummy_mean_mu, mean_mu);
//  MatrixMultiply(&T_A, &T_B, H, &dummy_one, H, cov_mu, dummy_mean_mu, mean_mu);
...
//      MatrixMultiply(&T_A, &T_B, G, G, &dummy_one, g1_vec, g2_vec, dummy_prec_g);
      MatrixMultiply(&T_A, &T_B, G, &dummy_one, G, &dummy_one, g1_vec, g2_vec, dummy_prec_g);
      for (g = 0; g < *G; g++) {
        for (h = 0; h < *G; h++) {
